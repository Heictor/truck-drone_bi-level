...
function main(nCust, nTrucks, nDrones)
    ...
    close all; clc;
    rng(1); % Set a fixed seed for reproducibility
    ...
    fprintf('Objective breakdown => Truck: $%.2f, Energy: $%.2f, DroneBonus: $%.2f, Total(est): $%.2f\n', ...
...
function inst = demoInstance(nCust, nTrucks, nDrones)
    ...
    inst.vTruck = 30; inst.vDrone = 50; inst.serv = 0.05; 
    ...
    % --- CHANGE: Calculate individual sortie flight times ---
    inst.sortieFlightTime = airDist / inst.vDrone; 
...
% --- CHANGE: New sophisticated scheduling logic ---
function sol=buildLaunchRecovery(inst, sol)
    sol.launchTimes=cell(inst.nDrones,1); 
    sol.recoverTimes=cell(inst.nDrones,1);
    
    drone_available_time = zeros(inst.nDrones, 1);
    drone_total_flight_time = zeros(inst.nDrones, 1);
    
    % Get flight times for all drone-served customers and sort them
    customer_indices = sol.droneCust;
    flight_times = inst.sortieFlightTime(customer_indices);
    
    [sorted_flight_times, sorted_order] = sort(flight_times, 'descend');
    sorted_customers = customer_indices(sorted_order);
    
    missions_to_schedule = numel(sorted_customers);
    
    for i = 1:missions_to_schedule
        % Find the drone that is available earliest
        [next_available_time, drone_to_use] = min(drone_available_time);
        
        cust_idx = sorted_customers(i);
        current_flight_time = sorted_flight_times(i);
        
        % Schedule the launch
        launch_time = max(next_available_time, inst.tw(cust_idx, 1));
        
        if launch_time + current_flight_time > inst.tw(cust_idx, 2)
            continue; % Skip if mission is impossible within time window
        end
        
        recovery_time = launch_time + current_flight_time;
        
        sol.launchTimes{drone_to_use}(end+1) = launch_time;
        sol.recoverTimes{drone_to_use}(end+1) = recovery_time;
        
        % Update this drone's availability and total work
        drone_available_time(drone_to_use) = recovery_time + inst.droneRechargeTime;
        drone_total_flight_time(drone_to_use) = drone_total_flight_time(drone_to_use) + current_flight_time;
    end
end
