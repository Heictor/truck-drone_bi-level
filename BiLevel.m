...
% Upper-Level: Drone Path Planning with A* Search and a physics-based PID
%              controller, optimized by a Bat Algorithm (BOA) to minimize
...
numDrones = numTrucks;
droneSpeed_H_base = 25.0; % Base horizontal speed in km/h (for zero payload)
droneSpeed_V_base = 10.0;  % Base vertical speed in km/h
maxPackageWeight = 2.0; % Max drone payload in kg
currentPackageWeight = 1.5; % Example payload for this run in kg
payloadSpeedFactor = 0.4; % Reduces max speed by 40% at max weight
...
% Calculate payload-adjusted drone speeds
...
cost = struct();
cost.truckPerKm = 1.50;         
cost.truckPerHour = 25.00;      
cost.dronePerHour = 20.00;      
cost.obstaclePenalty = 1000.00; 
cost.batteryPenalty = 0.10;    
cost.signalPenalty = 500.00;    
cost.dischargePenalty = 2000.00; % High penalty for overheating/damaging the battery
cost.maxDischargeRate = 0.5; % Max allowed battery discharge (units per hour)
...
for gen = 1:ga.maxGenerations
    ...
    parfor p = 1:ga.populationSize
        ...
        for t = 1:numTrucks
            ...
            % CORRECTED: Capture all 5 outputs to prevent assignment error
